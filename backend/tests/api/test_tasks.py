...
async def test_create_task(authed_client):
    resp = await authed_client.post(
        "/api/tasks", json={"category": "personal", "title": "Hello"}
    )
...
async def test_validation_empty_title(authed_client):
    resp = await authed_client.post(
        "/api/tasks", json={"category": "personal", "title": ""}
    )
...
async def test_patch_changes_status(authed_client):
    resp = await authed_client.post(
        "/api/tasks", json={"category": "personal", "title": "X"}
    )
...
async def test_reorder_endpoint(authed_client):
    a = (await authed_client.post("/api/tasks", json={"category": "personal", "title": "A"})).json()["task"]
    b = (await authed_client.post("/api/tasks", json={"category": "personal", "title": "B"})).json()["task"]
    c = (await authed_client.post("/api/tasks", json={"category": "personal", "title": "C"})).json()["task"]
    resp = await authed_client.post(
        f"/api/tasks/{c['id']}/reorder", json={"new_position": 0}
    )
...
async def test_user_isolation_on_get(client, test_user, other_user):
    ...
    created = await client.post(
        "/api/tasks", json={"category": "personal", "title": "Mine"}
    )
